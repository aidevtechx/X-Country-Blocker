...
(function () {
    ...
    function captureHeaders(headers) {
        ...
        headersReady = true;
        // console.log('[X Block] Captured Twitter API headers');
    }
    ...
    setTimeout(() => {
        if (!headersReady) {
            console.log('[X Block] No Twitter headers captured yet, using defaults');
            twitterHeaders = {
    ...
    window.addEventListener('message', async function (event) {
        ...
        if (event.data && event.data.type === '__fetchLocation') {
            ...
            try {
                ...
                // Using the Query ID from the reference repo: XRqGa7EeokUU5kppkh13EA
                ...
                console.error('[X Block] Error fetching location:', error);
                window.postMessage({
                    type: '__locationResponse',
                    screenName,
                    location: null,
